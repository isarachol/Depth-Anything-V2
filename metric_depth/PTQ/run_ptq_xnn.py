...
writer = SummaryWriter(args.outdir, filename_suffix='_ptq_{args.pyt_ver}_nonstrict_xnn')
...
def main():
    ...
    calibration_set = Subset(trainset, range(2)) # pick only the first 256 images for calibration
    ...
    # batch of args.bs images, each with 3 color channels and input resolution
    ...
    torch.save(quantized_model.state_dict(), os.path.join(args.outdir, f'{args.encoder}_{args.dataset}_pqt_{args.pyt_ver}_{args.strict}_xnn_v1.pth'))
    ...
    torch.save(quantized_model.state_dict(), os.path.join(args.outdir, f'{args.encoder}_{args.dataset}_pqt_opt_{args.pyt_ver}_{args.strict}_xnn_v1.pth'))
